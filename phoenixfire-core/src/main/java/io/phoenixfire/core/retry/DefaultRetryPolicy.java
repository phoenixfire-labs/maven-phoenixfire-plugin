 ...
 *       the configured ladder, and a crashed test is <em>never</em> retried in the shared pool that
 *       poisoned it - it jumps to at least {@link IsolationLevel#FRESH_FORK}.</li>
...
public final class DefaultRetryPolicy implements RetryPolicy {
    ...
    public RetryDecision decide(TestRecord record, FailureContext context) {
        ...
        if (context.failureMode().isInfrastructureFailure()) {
            ...
            // Never re-run a crashed test in the shared pool that may be poisoned.
