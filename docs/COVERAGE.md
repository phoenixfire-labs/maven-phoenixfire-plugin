...
JaCoCo is **off by default** (`jacoco.skip=true`). Enable the **95% line / 90% branch** aggregate gate with the **`coverage`** profile (`-Pcoverage`), which adds the `phoenixfire-coverage` module to the reactor. That module is **not** built or published on release `deploy`.
...
`phoenixfire-coverage/target/site/jacoco-aggregate/index.html` (only when `-Pcoverage` is active; runs after production modules in the reactor)
